...
import { INITIAL_CARS, INITIAL_METRICS, INITIAL_LOGS, US_LOCATIONS, RANDOM_NAMES } from './initialData';
import DashboardMetrics from './components/DashboardMetrics';
import InventoryCMS from './components/InventoryCMS';
import EventLogs from './components/EventLogs';
import ListingDetailModal from './components/ListingDetailModal';
import { BarChart3, Settings2, PlaySquare, ShieldAlert, Cpu, Lock, Unlock, ShieldCheck } from 'lucide-react';
...
export default function App() {
  ...
  const [dailyMetrics, setDailyMetrics] = useState<DailyMetricData[]>(INITIAL_METRICS);
  ...
  // Log appender helper
  const addLog = useCallback((type: ActivityLog['type'], carName: string, message: string, customLoc?: string, carId?: string) => {
    const loc = customLoc || US_LOCATIONS[Math.floor(Math.random() * US_LOCATIONS.length)];
  ...
  const handleAddCar = (newCar: Car) => {
    ...
    addLog('create', `${newCar.make} ${newCar.model}`, `CMS: Published new catalog listing for ${newCar.make} ${newCar.model} under ID ${newCar.id}`, newCar.seller.location, newCar.id);
  ...
  const handleUpdateCar = (updatedCar: Car) => {
    ...
    addLog('update', `${updatedCar.make} ${updatedCar.model}`, `CMS: Modified attributes/specifications of ${updatedCar.make} ${updatedCar.model}`, updatedCar.seller.location, updatedCar.id);
  ...
  const handleDeleteCar = (id: string) => {
    ...
    if (confirm(`Administrate CMS: Are you sure you want to permanently delete the listing for the ${target.make} ${target.model}?`)) {
      setCars(prev => prev.filter(c => c.id !== id));
      addLog('delete', `${target.make} ${target.model}`, `CMS: Revoked and unlisted vehicle ${target.make} ${target.model} from database pool`, target.seller.location);
  ...
  const handleSimulateView = (id: string) => {
    ...
    addLog('view', `${car.make} ${car.model}`, `Anonymous buyer from ${loc} viewed full layout config specs of ${car.make} ${car.model}`, loc, car.id);
  };
  ...
  const handleSimulateEnquiry = (id: string) => {
    ...
    const loc = US_LOCATIONS[Math.floor(Math.random() * US_LOCATIONS.length)];
    addLog('enquiry', `${car.make} ${car.model}`, `Sales Lead: ${name} sent buy enquiry to ${car.seller.name} for the ${car.make} ${car.model}`, loc, car.id);
  };

  // UPDATE CAR STATUS
  const handleUpdateCarStatus = (id: string, status: 'available' | 'sold' | 'archived') => {
    const target = cars.find(c => c.id === id);
    if (!target) return;
    ...
    <div className="min-h-screen bg-zinc-50/50 text-zinc-900 flex flex-col font-sans antialiased selection:bg-zinc-200">
      ...
      <header className="border-b border-zinc-200/60 bg-white/75 backdrop-blur-md sticky top-0 z-40 transition-all">
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5">
            <div className="bg-blue-50 border border-blue-100 p-2 rounded-lg text-blue-600 cursor-pointer" onClick={() => navigate('/')}>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto justify-end">
            <nav className="flex items-center p-0.5 bg-zinc-100/80 border border-zinc-200/55 rounded-lg" aria-label="Tabs navigation">
              ...
              <button
                ...
                id="tab_metrics"
              >
                <BarChart3 className={`w-3.5 h-3.5 ${(currentPath === '/' || currentPath === '/dashboard') ? 'text-white' : 'text-zinc-500'}`} />
                Metrics
              </button>
              ...
              <button
                ...
                id="tab_cms"
              >
                <Settings2 className={`w-3.5 h-3.5 ${currentPath === '/inventory' ? 'text-white' : 'text-zinc-500'}`} />
                Inventory CMS
              </button>
          ...
          {(currentPath === '/' || currentPath === '/dashboard' || currentPath === '') && (
            isAdmin ? (
              ...
              <div className="max-w-md w-full mx-auto my-12 p-8 text-center bg-white border border-zinc-200 shadow-md rounded-2xl animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <h3 className="text-base font-bold text-zinc-900">Administrator Access Required</h3>
                <p className="text-xs text-zinc-500 mt-2 leading-relaxed">
                  You are attempting to access the AutoDrive Metrics and System Event logs. Please sign in with your admin credentials to view interactive analytics, performance graphs, and catalog audit trails.
                </p>
                <button
                  onClick={() => navigate('/login')}
          ...
          {currentPath === '/inventory' && (
            isAdmin ? (
              ...
              <div className="max-w-md w-full mx-auto my-12 p-8 text-center bg-white border border-zinc-200 shadow-md rounded-2xl animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <h3 className="text-base font-bold text-zinc-900">Administrator Access Required</h3>
                <p className="text-xs text-zinc-500 mt-2 leading-relaxed">
                  You are attempting to access the Inventory CMS console. Please sign in with your admin credentials to publish new cars, manage catalogs, or adjust pricing parameters.
                </p>
                <button
                  onClick={() => navigate('/login')}
                  className="mt-6 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold cursor-pointer transition shadow-xs border border-transparent focus:outline-none"
            ...
            <div className="py-6">
              {isAdmin ? (
                <div className="max-w-md w-full mx-auto p-8 text-center bg-white border border-zinc-200 shadow-md rounded-2xl animate-in fade-in duration-300">
                  ...
                  <h3 className="text-base font-bold text-zinc-900">Session Verified</h3>
                  <p className="text-xs text-zinc-500 mt-2 leading-relaxed">
      ...
      <footer className="border-t border-zinc-200/60 bg-white py-8 mt-16 text-center text-xs text-zinc-400">
        <div className="max-w-7xl mx-auto px-6 space-y-1">
          <p className="font-mono text-zinc-500 text-[11px]">AutoDrive • Engineered with React &amp; Tailwind</p>
          <p className="text-[10px] text-zinc-400">Local state preservation for fast client-side performance auditing.</p>
        </div>
      </footer>
